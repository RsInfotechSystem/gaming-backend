const stockServices = require("../../db.services.js/stock.service");


const getStockStatusCount = async (request, response) => {
    try {
        //extract locationId from request
        const { locationId } = request;

        //get data from db & send response to client
        const stockStatusCount = await stockServices.getStockStatusCount(locationId);
        if (stockStatusCount) {
            return response.status(200).json({
                status: "SUCCESS",
                message: "Stock fetch successfully",
                stockStatusCount
            });
        } else {
            return response.status(200).json({
                status: "FAILED",
                message: "Stock not available",
            });
        }
    } catch (error) {
        return response.status(500).json({
            status: "FAILED",
            message: error.message
        })
    }
};


module.exports = getStockStatusCount;